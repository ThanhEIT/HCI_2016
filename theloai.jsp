...
<body class="nav-md">
	<%
		TheLoaiDAO thldao = new TheLoaiDAO();
	%>
								...
								<div class="x_title">
										...
										<table id="grid">
											<thead>
												<tr class="headings">
													...
													<th class="column-title">Tên thể loại>													
												</tr>
											</thead>
											<tbody>
												<%
													for (TheLoai thl : thldao.getAllTheLoai()) {
												%>
												<tr class="even pointer">
													<td>1</td>
													<td><%=thl.getMaTheLoai()%></td>
													<td><%=thl.getTenTheLoai()%></td>
									...
									<div id="dialog" class="gj-display-none">
										...
										<form>
											...
											<div class="gj-margin-top-10">
												<label for="tentg" class="gj-bold">Tên thể loại (*):</label>
  ...
  $(document).ready(function () {
    ...
    data = [
    { 'ID': 1, 'matg': 1001, 'tentg': 'Nguyễn Lê Phong'},
    { 'ID': 2, 'matg': 3678, 'tentg': 'Huỳnh Phước Ngà'},
    { 'ID': 3, 'matg': 1003, 'tentg': 'Trần Minh Tuấn'},
    { 'ID': 4, 'matg': 7443, 'tentg': 'Trần Ngọc Cường'},
    { 'ID': 5, 'matg': 1234, 'tentg': 'Nguyễn Thế Trường'},
    { 'ID': 6, 'matg': 1051, 'tentg': 'Lê Chí Bảo'},
    { 'ID': 7, 'matg': 3201, 'tentg': 'Âu Đình Bảo'},
    { 'ID': 8, 'matg': 5401, 'tentg': 'Phan Thị Hòa'},
    { 'ID': 9, 'matg': 6532, 'tentg': 'Phạm Văn Toàn'},
    { 'ID': 10, 'matg': 6333, 'tentg': 'Nguyễn Thanh Trí'},
    { 'ID': 11, 'matg': 6432, 'tentg': 'Đàm Quang Khoa'},
    { 'ID': 12, 'matg': 7577, 'tentg': 'Lê Thành Long'},
    { 'ID': 13, 'matg': 1965, 'tentg': 'Nguyễn Văn Tân'},
    { 'ID': 14, 'matg': 8652, 'tentg': 'Hoàng Thị Tốt'}
    ];
    grid = $('#grid').grid({
      dataSource: data,
      columns: [
      { field: 'ID',title:'STT', width: 40,align:'center' },
      { field: 'matg', title: 'Mã ', sortable: true ,width: 70,align:'center'},
      { field: 'tentg', title: 'Tên thể loại', sortable: true,align:'left' },
    ...
    function Save() {
      if ($('#ID').val()) {
        ...
        grid.updateRow(id, { 'ID': id, 'matg': $('#matg').val(), 'tentg': $('#tentg').val()});
      } else {
        grid.addRow({ 'ID': grid.count(true) + 1, 'matg': $('#matg').val(), 'tentg': $('#tentg').val()});
      }
      dialog.close();
    }
    ...
    $('#btnSearch').on('click', function () {
      ...
      result = $.grep(data, function (e) {
        return e.matg.indexOf(query) > -1 || e.tentg.indexOf(query) > -1;
      });
      grid.render(result);
    });
